...
| `before` | "end" 또는 **실재하는 녹화 씬 id** | 고급 (기본 숨김) | ★ 없는 씬을 가리키면 **클립이 조용히 버려진다** (아래) |
...
> ★ **`before` 함정 — 오류 없이 사라지는 클립.** `compose.js` 는 클립을 `before` 가 가리키는
> 녹화 씬 앞에 끼우고(`motionClips.filter(c => c.before === seg.id)`), **가리키는 씬이 없으면
> 그냥 버린다** — 빌드는 성공하고 그 구간만 없는 영상이 나온다. 엔진의 promo·manual 템플릿은
> 앱 화면 녹화용이라 클립이 `s1`·`s5` 에 묶여 있는데, 단발 종류는 녹화 씬을 걷어내므로
> (`scaffold._trim_clips` — 녹화할 웹앱이 없다) 그대로 두면 `before:"end"` 인 outro 한 장만
> 남는다. **2026-08-23 화면 실측에서 발견**: 홍보 1편이 3.2초 카드 한 장으로 완주했다.
> 그래서 스캐폴딩이 녹화 씬을 비울 때 남은 클립을 전부 `"end"` 로 옮긴다(배열 순서 = 재생 순서).
